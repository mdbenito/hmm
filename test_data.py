...
class TestMethods(ut.TestCase):
    ...
    def test_generate_trivial(self):
        ...
        d = data.generate_multinomial(N=N, M=M, L=100, p=p, A=A, B=B)
        self.assertTrue((d.Y == d.generator['Q']).all(),
                        "Trivial generation fails")
        # TODO: count

    def test_generate(self):
        d = data.generate_multinomial(N=4, M=10, L=100)
        for k, v in [[k, d.generator[k]] for k in ['p', 'A', 'B']]:
            self.assertTrue(is_row_stochastic(v),
                            "{0} doesn't define a probability: {1}".format(k, v))
